...
app.post('/api/user/lookup', auth, async (req, res) => {
  try {
    ...
    if (corePhone.startsWith('+254'))     corePhone = corePhone.slice(4);
    else if (corePhone.startsWith('254')) corePhone = corePhone.slice(3);
    else if (corePhone.startsWith('0'))   corePhone = corePhone.slice(1);

    // All possible formats stored in DB
    ...
    if (!user) return res.status(404).json({ message: 'No Nala account found for this number.' });
...
app.post('/api/transfer', auth, async (req, res) => {
  try {
    ...
    // ── Resolve recipient account — by username OR phone ──
    let recipUser = await User.findOne({ username: recipient.toLowerCase().trim() });
    if (!recipUser) {
      // Try phone lookup (any format)
      const cleanPhone = recipient.trim().replace(/[\s-]/g, '');
      let corePhone = cleanPhone;
      if (corePhone.startsWith('+254'))     corePhone = corePhone.slice(4);
      else if (corePhone.startsWith('254')) corePhone = corePhone.slice(3);
      else if (corePhone.startsWith('0'))   corePhone = corePhone.slice(1);
      const possiblePhones = [`0${corePhone}`, `254${corePhone}`, `+254${corePhone}`, cleanPhone];
      recipUser = await User.findOne({ phone: { $in: possiblePhones } });
    }
